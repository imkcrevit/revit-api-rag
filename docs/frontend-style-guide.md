...
本项目前端遵循 Graptolite BIM/AI 工具界面风格：近白背景、低对比线框、克制的 rust accent、Inter + IBM Plex Mono 字体，以及面向 Revit API / RAG 工作流的清晰信息密度。
...
- 页面背景使用 `--bg`，工具面板和卡片使用 `--panel`，辅助区域使用 `--bg2` 或 `--bg3`。
- 主体文字使用 `--serif`，标题使用 `--display`，技术标签、状态、日志、按钮和元信息使用 `--mono`。
- 按钮保持矩形和 2px 圆角；主按钮优先使用深色底，hover 时切换到 `--accent2`。
